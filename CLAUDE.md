...
- **Laravel AI SDK** (`laravel/ai`) for OpenAI integration
- **Pest 4** for testing
- **Tailwind CSS 4** with Vite
- **Tailwind Plus** - Tailwind UI components (properly licensed)
- **Spatie Laravel Permission** for roles/permissions
- **Laravel Actions** (`lorisleiva/laravel-actions`) for single-responsibility classes

## Architecture Notes

- Middleware configured in `bootstrap/app.php` via `withMiddleware()`
- Service providers listed in `bootstrap/providers.php`
- AI providers configured in `config/ai.php` (default: OpenAI)
- Uses SQLite by default (`database/database.sqlite`)

## Workflow Instructions

### Before starting work
- Enter plan mode to create a plan
- Write the plan to `.claude/tasks/TASK_NAME.md`
- Include implementation details, reasoning, and broken-down tasks
- Include unit tests when possible
- Research external packages/knowledge using Task tool
...
Activate relevant skills when working in these domains:
- `livewire-development` - Livewire components, wire: directives, reactivity
- `pest-testing` - Writing tests, assertions, mocking
- `tailwindcss-development` - Styling, responsive design, dark mode
- `laravel-permission-development` - Roles, permissions, policies
- `debugging-output-and-previewing-html-using-ray` - Ray debugging
